...
RSpec.describe User, type: :model do
  ...
  describe '.authenticate_with_credentials' do
    ...
    it 'check uniqueness of the email' do
      @user = User.create(:first_name=> "Gabriel", :last_name=> "Silva" , :password => "123456",  :password_confirmation => "123456", :email => "Gabriel.Silva@hotmail.com")
      @user1 = User.new(:first_name=> "Debora", :last_name=> "Fontana" , :password => "123456",  :password_confirmation => "123456", :email => "Gabriel.Silva@hotmail.com")
      @user1.valid?
      expect(@user1.errors.full_messages).to include "Email has already been taken"    
    end

    it 'returns user with right info' do
      @user = User.new(:first_name => 'name', :last_name => 'name', :password=> 'password1', :password_confirmation => 'password1', :email => 'gabriel@email.com')
      @user.save!
      expect(@user.authenticate_with_credentials('gabriel@email.com', 'password1')).to be_truthy
    end
